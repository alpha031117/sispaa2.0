...
const RightPanel = ({dummyData, pathname}: {dummyData: ComplatDetailType, pathname: string}) => {
    const downloadPath = `${pathname}/download/${dummyData.ID}.pdf`;
    ...
    return (
        <div className='flex flex-col items-center gap-y-12'>
            ...
            <div className='p-4 w-[20rem] rounded-md shadow-xl bg-gray-100'>
                <div>Assigned To:</div>
                <Link href={`/agency/${dummyData.agency_assigned}`}><div className='text-center m-3 '>🛑 {dummyData.agency_assigned}</div></Link>
            </div>

            {/* Complaint Report */}
            <div className='p-4 w-[20rem] rounded-md shadow-xl bg-gray-100'>
                <div>Complaint Report</div>
                <Link href={downloadPath}><div className='flex items-center justify-center m-3 '>
